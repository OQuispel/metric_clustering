...
# Code was adjusted to serve purpose of the research
# Errors: 'agreement' was incorrect because it ignored the last diagonal of the df leading to a lower agreement value
# Errors: 'total quantity' had a return within a loop instead of outside of it leading total quantity to always be 0
...
class pontiPy(object):
    ...
    def allocation(self):
        """Calculate the allocation difference without using the shift and exchange methods as this is very slow.
        """
        alloc = 0
        df_length = len(self.df_row_col_sums)-1                     
        for i in range(df_length):
            alloc += min(self.column_disagreement(i), self.row_disagreement(i))
        return alloc
    ...
    def exchange(self, category1 = None,category2 = None, Total = False):
        ...
        if category1 is None and Total is False:
            # Total exchange
            return sum(ex_by_category.values()) / 2
